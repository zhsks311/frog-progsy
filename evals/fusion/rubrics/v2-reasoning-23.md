...
Approximate prose is unsafe because an automatic exact or numeric grader needs a canonical machine-checkable value and cannot reliably infer intent from wording. The task should provide a plain integer, simplified fraction, or numeric value with an explicit tolerance, and the prompt should constrain the answer format. Ambiguous references should be replaced rather than left to grader interpretation.
