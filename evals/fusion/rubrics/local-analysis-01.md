...
Task-only fusion is cheap/safe but context-starved; full-context can solve history-dependent tasks but must exclude tools from pre-final stages; direct routing is simpler/low latency but lacks ensemble lift. Gate full-context with opt-in config, proxy-contract tests, and suite lift.
